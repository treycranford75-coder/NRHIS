...
function Invoke-SyntaxAndTestGates {
    ...
    $contractGate = Join-Path $RepoRoot "scripts\release\Invoke-NrhisWorkflowContractGate.ps1"
    if (-not (Test-Path $contractGate -PathType Leaf)) { throw "Missing workflow contract gate: $contractGate" }
    & $contractGate -BuildNumber $BuildNumber -RepositoryRoot $RepoRoot -EvidenceRoot $EvidenceRoot

    $parityGate = Join-Path $RepoRoot "scripts\release\Invoke-NrhisCiParityGate.ps1"
    if (-not (Test-Path $parityGate -PathType Leaf)) { throw "Missing CI parity gate: $parityGate" }
    & $parityGate -BuildNumber $BuildNumber -RepositoryRoot $RepoRoot -EvidenceRoot $EvidenceRoot
}
...
function Write-CiRepairGuidance {
    param(
        [Parameter(Mandatory)][string]$Repo,
        [Parameter(Mandatory)][object]$PullRequest,
        [Parameter(Mandatory)][string]$Build,
    ...
    New-Item -ItemType Directory -Path $diagnosticRoot -Force | Out-Null
    $stamp = [DateTime]::UtcNow.ToString("yyyyMMddTHHmmssZ")
    $logPath = Join-Path $diagnosticRoot ("pr-{0}-{1}-failed.log" -f $PullRequest.number, $stamp)
    $repairPath = Join-Path $diagnosticRoot ("pr-{0}-{1}-repair.ps1" -f $PullRequest.number, $stamp)

    $runJson = & gh run list --repo $Repo --branch $Branch --status failure --limit 5 --json databaseId,name,conclusion,url,headSha,event,createdAt 2>$null
    $runs = @()
    if ($runJson) {
        try { $runs = @($runJson | ConvertFrom-Json) } catch { $runs = @() }
    }
    ...
    foreach ($run in $runs) {
        $allLogs.Add("===== RUN $($run.databaseId): $($run.name) =====")
        $previousErrorActionPreference = $ErrorActionPreference
        $ErrorActionPreference = "Continue"
        try {
            $lines = & gh run view $run.databaseId --repo $Repo --log-failed 2>&1
        } finally {
            $ErrorActionPreference = $previousErrorActionPreference
        }
        foreach ($line in @($lines)) { $allLogs.Add([string]$line) }
    }
    $allLogs | Set-Content -Path $logPath -Encoding utf8

    $joined = ($allLogs -join "`n")
    $testNodes = [regex]::Matches($joined, '(?m)(tests/[A-Za-z0-9_./-]+\.py(?:::[A-Za-z0-9_\[\].-]+)*)') |
        ForEach-Object { $_.Groups[1].Value } | Select-Object -Unique

    $commands = New-Object System.Collections.Generic.List[string]
    $commands.Add('Set-Location (git rev-parse --show-toplevel)')
    $commands.Add('git status --short')
    if ($testNodes.Count -gt 0) {
        $quoted = $testNodes | ForEach-Object { '"' + $_ + '"' }
        $commands.Add('python -m pytest ' + ($quoted -join ' ') + ' -q')
    } elseif ($joined -match '(?i)ruff') {
        $commands.Add('ruff check .')
    } elseif ($joined -match '(?i)whitespace|blank line at EOF|trailing whitespace') {
        $commands.Add('git diff --check')
    } else {
        $commands.Add('python -m pytest -q')
    }
    $commands.Add('if ($LASTEXITCODE -ne 0) { throw "Local CI reproduction still fails." }')
    $commands.Add('Write-Host "Local reproduction passed. Apply the repair, amend the existing commit, and push with --force-with-lease." -ForegroundColor Green')
    $commands | Set-Content -Path $repairPath -Encoding utf8

    Write-Host "Failed-step log written: $logPath" -ForegroundColor Yellow
    Write-Host "Ready-to-run local reproduction: $repairPath" -ForegroundColor Yellow
    Write-Host "Run: & '$repairPath'" -ForegroundColor Cyan
    return $repairPath
}
        ...
        if ($null -ne $pr -and $pr.state -eq "OPEN") {
            ...
            if ($null -eq $pr -or $pr.state -ne "MERGED") {
                ...
                if ($null -ne $pr) {
                    ...
                    Write-CiRepairGuidance -Repo $Repository -PullRequest $pr -Build $BuildNumber -EvidenceRoot $EvidenceRoot | Out-Null
                    try { Start-Process $pr.url } catch { }
                }
