...
$script = Join-Path $repo 'scripts/analyze_lower_nueces_lag_stability.py'
if (-not $OutputRoot) {
...
if (-not (Test-Path $script -PathType Leaf)) { throw "Lower Nueces lag-stability script not found: $script" }

Write-Host 'NRHIS lower Nueces lag-stability and reach-residual analysis' -ForegroundColor Cyan
...
Write-Host 'Note:   lags and residuals are descriptive; no physical travel-time, water-balance, or causal claim.' -ForegroundColor Yellow
...
$argsList = @(
    ...
    '--rolling-step-days', [string]$RollingStepDays
)

& python @argsList
if ($LASTEXITCODE -ne 0) {
    throw "NRHIS lower Nueces lag-stability analysis failed with exit code $LASTEXITCODE."
}
