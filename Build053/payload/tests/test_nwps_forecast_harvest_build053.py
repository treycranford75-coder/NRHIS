...
def test_harvest_writes_forecasts_thresholds_and_receipt(tmp_path: Path, monkeypatch) -> None:
        ...
        json.dumps(
            {
                ...
                "gauges": [
                    {"identifier": "TEST1", "usgs_site_no": "00000001", "name": "Test Gauge"}
                ],
    ...
    stageflow = json.loads(
        (Path(__file__).parent / "fixtures" / "nwps_stageflow_build053.json").read_text(
            encoding="utf-8"
        )
    )
    metadata = {
    ...
    receipt = harvest(
        config, tmp_path / "out", retrieved_at=datetime(2026, 7, 19, 13, tzinfo=timezone.utc)
    )
    ...
    readiness = json.loads(
        (tmp_path / "out" / "nwps" / "nwps_readiness.json").read_text(encoding="utf-8")
    )
...
def test_station_failure_is_isolated(tmp_path: Path, monkeypatch) -> None:
    config = tmp_path / "config.json"
    config.write_text(
        json.dumps({"gauges": [{"identifier": "BAD", "name": "Bad Gauge"}]}), encoding="utf-8"
    )
