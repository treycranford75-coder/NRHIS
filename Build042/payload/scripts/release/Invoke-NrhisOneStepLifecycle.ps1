...
function Invoke-Git([string[]]$Arguments, [string]$FailureMessage) {
    ...
    try {
        ...
        do {
            $attempt++
            # Git for Windows can prompt when OneDrive or another process keeps a
            # directory locked during checkout. Feed "n" answers noninteractively:
            # the checkout continues, retained tracked artifacts stay untouched,
            # and the lifecycle no longer waits for keyboard input.
