...
if ($repo -match '(?i)[\\/]OneDrive[\\/]') {
    if (-not (Test-Path $migrationScript -PathType Leaf)) { throw "Build061 repository migration helper is missing." }
    & $migrationScript -SourceRepository $repo -DestinationRepository $MigrationDestination -BuildNumber "061" -NoChain:$NoChain
    return
}
$orchestratorSource = Join-Path $payload "scripts\release\Invoke-NrhisOneStepLifecycle.ps1"
if (-not (Test-Path $orchestratorSource -PathType Leaf)) { throw "Build061 lifecycle helper is missing." }
$asset = Join-Path $repo "NRHIS_Sprint2_Build061_OneStep.zip"
$params = @{
    BuildNumber = "061"
    Repository = "treycranford75-coder/NRHIS"
    Branch = "feature/sprint2-build061"
    CommitMessage = "Build061: add twice-daily publication bundle and QA release gate"
    PullRequestTitle = "Build061: add twice-daily publication bundle and QA release gate"
    PullRequestBodyFile = (Join-Path $payload "docs\releases\BUILD061_PR.md")
    Tag = "v0.1.1-rc61+build061"
    ReleaseTitle = "NRHIS Sprint 2 Build061 - Twice-Daily Publication Bundle"
    ReleaseNotesFile = (Join-Path $payload "docs\releases\BUILD061_RELEASE_NOTES.md")
    PayloadRoot = $payload
