...
from nrhis_analysis.lower_nueces_flow_state_transition import (  # noqa: E402
    analyze_lower_nueces_flow_state_transition,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Analyze lower Nueces discharge coherence thresholds, coherent-flow "
            "events, and statistically aligned residuals."
        )
    )
...
def main() -> int:
    ...
    output_dir = args.output_root.resolve() / f"lower-nueces-flow-state-{stamp}"
    result = analyze_lower_nueces_flow_state_transition(
        args.csv.resolve(),
        args.index.resolve(),
        output_dir,
        start=args.start,
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"NRHIS lower Nueces flow-state analysis failed: {exc}") from exc
