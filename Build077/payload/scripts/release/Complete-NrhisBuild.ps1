...
$repository = (& "$repo\scripts\release\Get-NrhisGitHubRepository.ps1").Trim()
if ([string]::IsNullOrWhiteSpace($repository)) {
    throw 'Unable to determine the GitHub repository.'
}

$existing = & gh release view $Tag --repo $repository --json url --jq '.url' 2>$null
if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace([string]$existing)) {
    $releaseUrl = ([string]$existing).Trim()
