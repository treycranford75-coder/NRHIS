...
$params = @{
    ...
    NotesFile = (Join-Path $repo "docs\releases\BUILD062_RELEASE_NOTES.md")
...
if (-not $NoArchive) {
    ...
    if (Test-Path $archive -PathType Leaf) {
        & $archive -BuildNumber "062" -RepositoryRoot $repo
    }
}
Write-Host "Build062 completed and verified." -ForegroundColor Green
if (-not $NoChain) {
    Write-Host "lets move on to next build" -ForegroundColor Yellow
    Write-Host "Waiting for Build063 root ZIP." -ForegroundColor Yellow
}
