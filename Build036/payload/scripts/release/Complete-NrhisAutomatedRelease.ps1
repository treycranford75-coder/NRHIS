...
if ($null -eq $release) {
    ...
    if ($canUseGh) {
        ...
        Write-Host "Rerun this same command after publication; verification will finish automatically."
        return
    }
...
if ($resolvedCommit -ne $expectedCommit) {
    throw "Release tag commit mismatch. Expected '$expectedCommit'; found '$resolvedCommit'."
}
...
if (-not $expectedCommit.StartsWith(([string]$receipt.verified_commit).Trim(), [StringComparison]::OrdinalIgnoreCase) -and
    -not ([string]$receipt.verified_commit).Trim().StartsWith($expectedCommit, [StringComparison]::OrdinalIgnoreCase)) {
