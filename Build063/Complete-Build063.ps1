...
$params = @{
    ...
    NotesFile = (Join-Path $repo "docs\releases\BUILD063_RELEASE_NOTES.md")
...
if (-not $NoArchive) {
    ...
    if (Test-Path $archive -PathType Leaf) {
        & $archive -BuildNumber "063" -RepositoryRoot $repo
    }
}
Write-Host "Build063 completed and verified." -ForegroundColor Green
if (-not $NoChain) {
    Write-Host "lets move on to next build" -ForegroundColor Yellow
    Write-Host "Waiting for Build064 root ZIP." -ForegroundColor Yellow
}
