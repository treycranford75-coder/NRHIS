...
def build_snapshot(
    config_path: Path, data_root: Path, *, generated_at: datetime | None = None
) -> dict[str, Any]:
    ...
    quality = (
        ...
        else {"ready_for_reporting": False, "stations": []}
    )
    ...
    quality_by_site = {
        str(row.get("site_no", "")): row
        for row in quality.get("stations", [])
    ...
    for station in stations:
        ...
        missing_required = [
            code for code in station.get("required_parameters", []) if code not in obs
        ]
        ...
        rows.append(
            {
                ...
                "latest_observed_at": max(
                    (str(v.get("observed_at", "")) for v in obs.values()), default=""
                ),
    ...
    ready = (
        not blocking
        and bool(nwps.get("ready_for_reporting", False))
        and bool(quality.get("ready_for_reporting", False))
    )
    ...
    result = {
        ...
        "highest_forecast_category": max(
            (r["forecast_category"] for r in rows), key=_severity, default="not_defined"
        ),
        "stations": rows,
    }
    ...
    _write_csv(
        alerts_path,
        sorted(
            [r for r in rows if r["alert_score"] > 0],
            key=lambda r: (-r["alert_score"], r["display_order"]),
        ),
        fields,
    )
    _atomic_write(
        ...
        json.dumps(
            {
                ...
                for k in (
                    ...
                    "forecast_alert_count",
                    "highest_forecast_category",
                )
            },
            indent=2,
            sort_keys=True,
