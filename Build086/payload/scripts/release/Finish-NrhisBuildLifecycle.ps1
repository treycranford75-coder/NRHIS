...
if ($state -ne 'MERGED') {
    Write-LifecycleState -Phase 'ci' -Status 'running' -Detail 'Waiting for required CI checks to register and complete.'

    $checkRegistrationAttempts = 12
    $checkRegistrationDelaySeconds = 10
    ...
    for ($attempt = 1; $attempt -le $checkRegistrationAttempts; $attempt++) {
        ...
        if ($checkText -match 'no checks reported') {
            ...
            Write-LifecycleState -Phase 'ci' -Status 'pending' -Detail 'CI checks did not register before the retry window expired.'
            Write-Host "Resume with: .\scripts\release\Resume-NrhisBuildLifecycle.ps1 -BuildNumber '$BuildNumber'" -ForegroundColor Yellow
            throw "Build$BuildNumber CI checks have not registered. Automatic merge remains pending."
        }
    ...
    if (-not $checksRegistered) {
        throw "Build$BuildNumber CI registration state could not be determined."
    }

    Write-Host 'Monitoring required CI checks...' -ForegroundColor Cyan
    & gh pr checks $PullRequestUrl --watch --fail-fast
    if ($LASTEXITCODE -ne 0) {
        Write-LifecycleState -Phase 'ci' -Status 'failed' -Detail 'One or more required CI checks failed.'
        Write-Host "Resume with: .\scripts\release\Resume-NrhisBuildLifecycle.ps1 -BuildNumber '$BuildNumber'" -ForegroundColor Yellow
