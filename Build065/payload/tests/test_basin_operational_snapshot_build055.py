...
FIX = Path(__file__).parent / "fixtures" / "build055"
...
def _setup(tmp_path: Path) -> tuple[Path, Path]:
    ...
    shutil.copy(
        FIX / "usgs_current_conditions.json", data / "current" / "usgs_current_conditions.json"
    )
    shutil.copy(FIX / "nwps_readiness.json", data / "nwps" / "nwps_readiness.json")
    shutil.copy(
        FIX / "usgs_data_quality_summary.json", data / "quality" / "usgs_data_quality_summary.json"
    )
                ...
                "stations": [
                    {
                        ...
                        "name": "Three Rivers",
                        "display_order": 1,
                        "required_parameters": ["00060", "00065"],
                    },
                    {
                        "site_no": "08211200",
                        "name": "Bluntzer",
                        "display_order": 2,
                        "required_parameters": ["00060", "00065", "00095"],
                    },
                ]
            }
        ),
...
def test_missing_required_parameter_blocks_reporting(tmp_path: Path) -> None:
    ...
    (data / "current" / "usgs_current_conditions.json").write_text(
